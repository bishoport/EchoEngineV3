...
namespace libCore
{
    ...
    //--SERIALIZATION privates
    void libCore::SceneManager::SerializeScene()
	{
        YAML::Emitter out;
        out << YAML::BeginMap;
        ...
        std::ofstream fout("assets/Scenes/" + currentScene->name + ".yaml");
        fout << out.c_str();
	}
	void SceneManager::DeserializeScene(std::string _sceneName)
	{
            ...
            for (auto entityNode : entities) {
                ...
                if (entityNode["MeshComponent"]) {
                    ...
                    if (model != nullptr) {
                        ...
                        ConsoleLog::GetInstance().AddLog(LogLevel::L_ERROR, "ERROR AL CARGAR EL MODELO con la mesh:" + std::string(meshModelName));
                        continue;
                    }
                }
	...
	void SceneManager::SerializeComponents()
	{
        YAML::Emitter out;
        out << YAML::BeginMap;
        out << YAML::Key << "Scene" << YAML::Value << currentScene->name + "_temp";
        ...
        std::ofstream fout("assets/Scenes/" + currentScene->name + "_temp.yaml");
        fout << out.c_str();
	}
	void SceneManager::DeserializeComponents()
	{
        YAML::Node data = YAML::LoadFile("assets/Scenes/" + currentScene->name + "_temp.yaml");
        if (!data["Scene"]) {
            return;
        }
